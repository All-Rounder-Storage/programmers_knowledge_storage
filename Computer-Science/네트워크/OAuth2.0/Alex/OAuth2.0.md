...
- `📢 일반적인 인증 흐름을 간략하게 알아보고 아래에서 실제 권한부여 방식을 자세히 알아보겠다`
  1. 소비자 → 서비스 제공자 (요청토큰 요청)
  2. 서비스 제공자 → 소비자 (요청토큰 발급)
  3. 소비자가 사용자를 서비스 제공자로 이동 (사용자 인증 수행)
  4. 서비스 제공자가 사용자를 소비자로 이동
  5. 소비자가 접근토큰을 요청
  6. 서비스 제공자가 접근토큰 발급 (Access Token)
  7. 발급된 접근토큰을 이용하여 소비자에서 사용자 정보(자원)에 접근
- `3, 4번을 아래 그림의 Authorization server에서 수행`
...
- 접근 자격이 있는지 검증하는 서버 (로그인)
- 권한 서버에 권한을 요청하기 전 로그인 주체가 누구인지 판별하는 서버

### Authorization Server (인가 서버)

- 권한 관리 및 부여 서버
- 자원 접근 권한을 위임 및 관리하는 서버
...
- 자원에 대한 접근 권한을 자원 소유자가 인가(Authorization) 하였음을 나타내는 자격 증명
- 만료기간이 존재 (Life-Time)
...
- 하지만 API 호출 요청이 들어올때마다 전달 받은 Access Token이 유효한지 계속 확인해야 한다면 어떨까?
- MSA 아키텍쳐로 구성되어 있는 많은 서비스에서 각각의 서버가 Access Token 유효성 검사를 매 요청마다 한다면, 인가(Authorization)서버에서 버티지 못하고 병목 현상 등이 발생하여 서버 부하로 이어질 것이다.
